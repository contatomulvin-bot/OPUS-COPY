...
export class PipelineService {
    ...
    try {
      ...
      if (segments.length === 0 || forceRetry) {
        ...
        const transcriptRecord = await prisma.transcript.upsert({
          ...
          update: { text: transcriptionResult.text, language: transcriptionResult.language },
          create: { videoId: video.id, text: transcriptionResult.text, language: transcriptionResult.language },
        });
        ...
        if (transcriptionResult.segments.length > 0) {
          await prisma.transcriptSegment.createMany({
            data: transcriptionResult.segments.map(s => ({ transcriptId: transcriptRecord.id, startTime: s.startTime, endTime: s.endTime, text: s.text })),
          });
        }
      ...
      const aiCandidateClips = await defaultClipAnalyzerService.findBestMoments(
        segments.map(s => ({ id: s.id, startTime: s.startTime, endTime: s.endTime, text: s.text })),
        { videoDuration: metadata.duration, videoTitle: video.originalName }
      );
      ...
      for (const candidate of aiCandidateClips) {
        createdClips.push(await prisma.clip.create({
          data: {
            videoId: video.id,
  ...
  async renderClip(clipId: string, options: RenderClipOptions = {}): Promise<{ renderId: string; job: JobProgress }> {
    const clip = await prisma.clip.findUnique({
      where: { id: clipId },
      include: { video: { include: { transcript: { include: { segments: { orderBy: { startTime: 'asc' } } } } } } },
    });
  ...
  private async executeClipRender(clipId: string, renderId: string, jobId: string, options: RenderClipOptions) {
    try {
      ...
      if (options.subtitlesEnabled !== false && clipSegments.length > 0) {
        const captions = clipSegments.map(segment => ({
          startTime: Math.max(0, segment.startTime - clip.startTime),
          endTime: Math.max(0, segment.endTime - clip.startTime),
          text: segment.text,
        }));
      ...
      await defaultVideoProcessor.renderVertical({
        ...
        subtitlesPath: options.subtitlesEnabled !== false && clipSegments.length > 0 ? absoluteSubtitlesPath : undefined,
        subtitlesEnabled: options.subtitlesEnabled !== false && clipSegments.length > 0,
        subtitleStyle,
      ...
      const stat = await fs.promises.stat(absoluteOutputPath);
      await prisma.render.update({ where: { id: renderId }, data: { status: 'COMPLETED', outputPath: outputRelativePath, fileSize: BigInt(stat.size), duration: validation.metadata.duration, width: validation.metadata.width, height: validation.metadata.height } });
