...
def _ass_time(seconds: float) -> str:
    value = max(0, int(round(seconds * 100)))
...
def write_ass(transcript: dict, clip: ClipCandidate, path: Path, style: SubtitleStyle) -> None:
    position = max(5, min(95, int(style.vertical_position)))
    y = int(round(1920 * position / 100))
    alignment = 5
    back_alpha = max(0, min(100, int(style.background_opacity)))
    back_alpha_ass = 255 - round(back_alpha * 2.55)
    back_color = _ass_color(style.background_color, "#000000")
    text_color = _ass_color(style.text_color, "#FFFFFF")
    outline_color = _ass_color(style.outline_color, "#000000")
    ...
    lines = [
        ...
        f"Style: Default,{style.font_family},{max(10, int(style.font_size))},{text_color},{text_color},{outline_color},&H{back_alpha_ass:02X}{back_color[4:]},{bold},0,0,0,100,100,0,0,3,{max(0, int(style.outline_width))},{max(0, int(style.shadow))},{alignment},40,40,0,1",
        "",
        "[Events]",
    ...
    for segment in transcript.get("segments", []):
        ...
        if words:
            ...
            for word in words:
                ...
                if len(group) >= 7:
                    lines.append(
                        f"Dialogue: 0,{_ass_time(group_start - clip.start)},{_ass_time(group_end - clip.start)},Default,,0,0,0,,{{\\pos(540,{y})}}{_ass_escape(' '.join(group))}"
                    )
            ...
            if text:
                s = max(start, clip.start) - clip.start
                e = min(end, clip.end) - clip.start
                if e > s:
                    lines.append(
                        f"Dialogue: 0,{_ass_time(s)},{_ass_time(e)},Default,,0,0,0,,{{\\pos(540,{y})}}{_ass_escape(text)}"
                    )

    path.write_text("\n".join(lines) + "\n", encoding="utf-8-sig")


# Kept for compatibility with any external callers that still expect SRT output.
...
class ClipRenderer:
    def __init__(self, subtitle_style: SubtitleStyle | None = None) -> None:
    ...
    def _render(
        ...
        write_ass(transcript, clip, ass, self.subtitle_style)
