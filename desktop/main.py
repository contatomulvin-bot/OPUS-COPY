...
QFrame#shell, QFrame#card { background:#101010; border:1px solid #242424; border-radius:18px; }
...
QLineEdit, QSpinBox, QDoubleSpinBox, QComboBox { background:#080808; border:1px solid #303030; border-radius:10px; padding:10px 12px; color:#f5f5f5; min-height:20px; }
QLineEdit:focus, QSpinBox:focus, QDoubleSpinBox:focus, QComboBox:focus { border:1px solid #686868; }
...
QPushButton#colorButton { min-width:80px; }
...
class VideoCard(QFrame):
    ...
    def toggle(self) -> None:
        ...
        if state in (QMediaPlayer.PlaybackState.PlayingState, QMediaPlayer.PlaybackState.PausedState):
            self.player.pause() if state == QMediaPlayer.PlaybackState.PlayingState else self.player.play()
        else:
            self.player.play()
...
class MainWindow(QMainWindow):
    ...
    def build_creation_tab(self) -> QWidget:
        ...
        desc = QLabel("Cole o YouTube, escolha onde salvar e monte o visual das legendas antes de gerar."); desc.setObjectName("muted"); desc.setWordWrap(True)
    ...
    def make_color_button(default: str) -> QPushButton:
        ...
        def choose():
            color = QColorDialog.getColor(QColor(button.property("color")), button.window(), "Escolher cor")
            if color.isValid():
                value = color.name().upper(); button.setText(value); button.setProperty("color", value); button.setStyleSheet(f"background:{value}; color:{'#000000' if sum(color.getRgb()[:3]) > 420 else '#FFFFFF'}; border:1px solid #555;")
        button.clicked.connect(choose)
        button.setStyleSheet("background:#000000; color:#FFFFFF; border:1px solid #444;") if default == "#000000" else button.setStyleSheet("background:#FFFFFF; color:#000000; border:1px solid #555;")
        return button
    ...
    def start_pipeline(self) -> None:
        ...
        if not url:
            ...
            return
        self.start.setEnabled(False); self.choose_output.setEnabled(False); self.status.setText("Preparando análise…")
        self.thread = QThread(); self.worker = Worker(url, self.count.value(), self.output_dir, self.subtitle_style()); self.worker.moveToThread(self.thread)
    ...
    def failed(self, message: str) -> None:
        ...
        box = QMessageBox(self); box.setIcon(QMessageBox.Icon.Critical); box.setWindowTitle("OPUS-COPY — erro"); box.setText("O processamento falhou."); box.setDetailedText(message)
        copy_button = box.addButton("Copiar erro completo", QMessageBox.ButtonRole.ActionRole); box.addButton(QMessageBox.StandardButton.Close); box.exec()
